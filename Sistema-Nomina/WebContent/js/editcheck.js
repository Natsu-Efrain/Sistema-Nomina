             ...
             switch (cmdarr[0]) {
               ...
               case "type" : 
                   switch (cmdarr[1].toLowerCase()) {
                       ...
                       case "date" : 
                           if (!msg || !msg.length) 
                              msg = ""+ " Debe ser una fecha en formato MM/DD/AA" + ((fld.size>=10 || fld.maxlength>=10)?"YY":"") + "!";
		              ...
		              if (    parseInt(aDateParts[0],10) < 1 || parseInt(aDateParts[0],10) > 12
                       ...
                       case "alpha":  
                         if(fld.value.length>0 && fld.value.search("[^A-Z a-z]") >= 0) { 
                              if (!msg || !msg.length)
                                 msg = "Solo estan permitidos los caracteres alfabanumericos!";
                              window.alert(msg);
                              fld.focus();
                              fld.select();
                              return false;
                       ...
                       case "alphanum":  { 
                         if(fld.value.length>0 && fld.value.search("[^A-Z a-z0-9]") >= 0) { 
                              if (!msg || !msg.length)
                                 msg = "Solo estan permitidos los caracteres alfanumericos!";
                              window.alert(msg);
                              fld.focus();
                              fld.select();
                              return false;
                       ...
                       case "email" : 
                           ...
                           if (fld.value.length>0 && re.test(fld.value) == false) {
                              if (!msg || !msg.length)
                                 msg = "La direccion de correo electronico no es valida!";
                              window.alert(msg);
                              fld.focus();
                              fld.select();
                              return false;
                       ...
                       case "zip" : 
                           ...
                           if (fld.value.length>0 && re.test(fld.value) == false) {
                              if (!msg || !msg.length)
                                 msg = "El codigo postal no es valido!";
                              window.alert(msg);
                              fld.focus();
                              fld.select();
                              return false;
                       ...
                       case "ssn": 
                            if(fld.value.length>0 && !(fld.value.match(/^\d{3}-?\d{2}-?\d{4}$/))) {
                              if (!msg || !msg.length)
                                 msg = "El numero de Seguro Social no es valido!";
                              window.alert(msg);
                              fld.focus();
                              fld.select();
                              return false;
                       ...
                       case "state": 
                            if (fld.value.length>0 && !(fld.value.toUpperCase().match(/^AL|AK|AS|AZ|AR|CA|CO|CT|DE|DC|FM|FL|GA|GU|HI|ID|IL|IN|IA|KS|KY|LA|ME|MH|MD|MA|MI|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|MP|OH|OK|OR|PW|PA|PR|RI|SC|SD|TN|TX|UT|VT|VI|VA|WA|WV|WI|WY|AE|AA|AE|AE|AP$/))) {
                              if (!msg || !msg.length)
                                 msg = "El estado estadounidense no es valido!";
                              window.alert(msg);
                              fld.focus();
                              fld.select();
                              return false;
                       ...
                       default:  // regular expression
                         ...
                         if (fld.value.length>0 && re.test(fld.value) == false) {
                              if (!msg || !msg.length)
                                 msg = ""+ " no cumple con los criterios de validacion!";
                              window.alert(msg);
                              fld.focus();
                              fld.select();
                              return false;
               ...
               case "maxlen" : 
                   if(fld.value.length && fld.value.length>0 && (fld.value.length > eval(cmdarr[1]))) {
                         if (!msg || !msg.length)
                             msg = "La extension maxima de " + ""+ " es " + cmdarr[1] + " caracteres!";
                         window.alert(msg);
                         fld.focus();
                         fld.select();
                         return false;
               ...
               case "minlen" : 
                   if(fld.value.length && fld.value.length>0 && (fld.value.length < eval(cmdarr[1]))) {
                         if (!msg || !msg.length)
                             msg = " La extension minima de " + ""+ " es " + cmdarr[1] + " carecteres!";
                         window.alert(msg);
                         fld.focus();
                         fld.select();
                         return false;
               ...
               case "minval" : 
                   if(fld.value.length && fld.value.length>0 && (eval(fld.value) < eval(cmdarr[1]))) {
                         if (!msg || !msg.length)
                             msg = "El valor minimo de " + ""+ " es " + cmdarr[1] + "!";
                         window.alert(msg);
                         fld.focus();
                         fld.select();
                         return false;
               ...
               case "maxval" : 
                   if(fld.value.length && fld.value.length>0 && (eval(fld.value) > eval(cmdarr[1]))) {
                         if (!msg || !msg.length)
                             msg = "El valor maximo de " + ""+ " es " + cmdarr[1] + "!";
                         window.alert(msg);
                         fld.focus();
                         fld.select();
                         return false;
